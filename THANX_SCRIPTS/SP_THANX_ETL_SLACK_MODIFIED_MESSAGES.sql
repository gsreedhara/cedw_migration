...
/****** Object:  StoredProcedure [dbo].[SP_THANX_ETL_SLACK_MODIFIED_MESSAGES]    Script Date: 4/19/2024 9:50:45 AM ******/
SET ANSI_NULLS ON
GO
...
-- * *********************************************************************************
-- * File Name        : PXLTD_CEDW.dbo.[SP_THANX_ETL_SLACK_MODIFIED_MESSAGES]
-- *
-- * Description      :    
-- *                   Step1: 
-- *                   Step2: 
-- *				   Step3: 
-- *
-- * Purpose          : Get modified Slack messages from Stage table and update the Load Tables
-- *                    
...
-- * v1.0	   	20Jan2019					Code created
-- * *********************************************************************************
CREATE PROCEDURE [dbo].[SP_THANX_ETL_SLACK_MODIFIED_MESSAGES] (@days int)
AS
BEGIN
	...
	-- * *********************************************************************************
	--From #STG1, select all messages loaded before through ETL process, and modified on slack after loaded
	--Select only messages (exclude threads)
	--Considering only message text modified. Reactions are not implemented yet
	SELECT STG.*, MSG.RCGNTN_MSG_ID
	INTO #STG2
	FROM #STG1 STG
	INNER JOIN PXLTD_CEDW.dbo.RCGNTN_MSG MSG
		ON STG.SRC_TS = MSG.SRC_TS
		AND STG.INTRNL_TS > MSG.INTRNL_TS
		AND STG.FRM_USR_SRC_ID = MSG.FRM_USR_SLACK_ID
	WHERE (STG.SRC_THRD_TS IS NULL OR STG.SRC_TS = STG.SRC_THRD_TS) --excluding threads
	AND (STG.EDITED IS NOT NULL) --Add here "OR STG.REACTN_SCTN <> reactions from MSG" when implemented!
	...
	-- * *********************************************************************************
	--Updating messages from the RCGNTN_MSG table
	UPDATE MSG SET 
		MSG.INTRNL_TS = STG.INTRNL_TS,
		MSG.RCGNTN_MSG_TXT = STG.NEW_MSG_TEXT, 
		MSG.RCGNTN_TAG_ID = TAG.RCGNTN_TAG_ID
		--Insert here an UPDATED_DATE column
	--SELECT STG.*, MSG.*, TAG.TAG_CD
	FROM #STG2 STG
	INNER JOIN PXLTD_CEDW.dbo.RCGNTN_MSG MSG
		ON STG.RCGNTN_MSG_ID = MSG.RCGNTN_MSG_ID
	INNER JOIN PXLTD_CEDW.dbo.RCGNTN_TAGS_TYP TAG 
		ON TAG.TAG_CD = STG.TAG_CD;

	-- * *********************************************************************************
	--Updating the TO_USERS from the RCGNTN_MSG_TO_USR table
	UPDATE #STG2 SET TO_USR_SRC_ID = TO_USR_SRC_ID + '|' WHERE TO_USR_SRC_ID <> '';

	DELETE FROM PXLTD_CEDW.dbo.RCGNTN_MSG_TO_USR 
	WHERE RCGNTN_MSG_ID IN 
		(SELECT RCGNTN_MSG_ID FROM #STG2);
	...
	BEGIN 
		...
		BEGIN
			...
			SELECT STG.RCGNTN_MSG_ID, @src_id_found, @emp_id_found
			FROM #STG2 STG
	...
	BEGIN 
		...
		WHILE CHARINDEX('|', @value_cd_, @pos1_) > 0 
			BEGIN
				...
				SELECT STG.RCGNTN_MSG_ID, VAL.RCGNTN_VALUE_ID
				FROM #STG2 STG
