...
def create_connection(driver,server_name,db_name):
    cnx = pyodbc.connect("Driver= {%s};"
                                "Server=%s;"
                                "Database=%s;"
                                "Trusted_Connection=yes;" % (driver,server_name,db_name))
    
    #Define the cursor that will be used later in the script 
...
def run_queries(connection):
    ...
    RCGNTN_MSG_WRK WHERE PRCSSD_IND ='N'"
    ...
    query_to = "SELECT RCGNTN_MSG_ID,RCGNTN_TO_USR_EMAIL FROM RCGNTN_MSG_TO_WRK"
    to_df = pd.read_sql(query_to,connection)
    #Run SQL query to pull dataframe with Company Values included in Message
    query_values = "SELECT RCGNTN_MSG_ID,RCGNTN_VALUES_DESC FROM RCGNTN_MSG_VALUE_WRK WHERE \
    RCGNTN_MSG_ID IN (SELECT RCGNTN_MSG_ID FROM RCGNTN_MSG_WRK)"
...
def post_message_and_add_reactions(connection,df_with_usernames,slack_token,channel_name):
    ...
    for index in df_with_usernames["RCGNTN_MSG_ID"].sort_values(ascending=True).unique():
        ...
        channels_list = sc.api_call(method="channels.list")
        ...
        channel_history = sc.api_call(method="channels.history",channel=str(channel_id))
...
def delete_day_old_rows(connection):
    ...
    cursor.execute("DELETE FROM RCGNTN_MSG_WRK WHERE RCGNTN_TS < GETDATE()-1 AND PRCSSD_IND = 'Y'")
    cursor.execute("DELETE FROM RCGNTN_MSG_TO_WRK WHERE RCGNTN_TS < GETDATE()-1 AND RCGNTN_MSG_ID NOT IN (SELECT RCGNTN_MSG_ID \
        FROM RCGNTN_MSG_WRK)")
    cursor.execute("DELETE FROM RCGNTN_MSG_VALUE_WRK WHERE RCGNTN_TS < GETDATE()-1 AND RCGNTN_MSG_ID NOT IN (SELECT RCGNTN_MSG_ID \
        FROM RCGNTN_MSG_WRK)")
    connection.commit()
    cursor.close()
    #close connection
...
def main():
    create_connection(driver,server_name,db_name)
    connection = create_connection(driver,server_name,db_name)
