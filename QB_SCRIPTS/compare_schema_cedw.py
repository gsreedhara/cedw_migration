...
def standardiseColumnType(dbConn, table_name):
    ...
    for row in column_rows:
        ...
        column_list.append([column_name, column_type, row[2], row[3], row[4]])
...
def compare_column_names(table_name, src_column_names, tgt_column_names):
    ...
    """
    diff_column_str = table_name + ' '

    ''' Foreach entry in the source columns lookup the value in the target
        table '''
    for src_name, src_type, src_size, src_precision, src_scale in src_column_names:
        column_found_ind = False

        for tgt_name, tgt_type, tgt_size, tgt_precision, tgt_scale in tgt_column_names:
            if src_name == tgt_name:
                ...
                if src_type != tgt_type:
                    if src_type == 'BIT' and tgt_type in ('SMALLINT',
                        ...
                        diff_column_str += (f'\nDATA TYPE MISMATCH - {src_name} {src_type} '
                            f' {src_size} {src_precision} {src_scale} '
                            f' New Type {tgt_type} {tgt_size} {tgt_precision} {tgt_scale}')
                elif int(src_size) > int(tgt_size) and src_type not in (
                        'BIGINT', 'FLOAT', 'DATE', 'INTEGER', 'SMALLINT'):
                    if src_type == 'NUMERIC' and int(src_size) - int(tgt_size) > 1:
                        print('Numeric Data Size Mismatch ', src_name, src_type, src_size, tgt_size)
                        diff_column_str += (f'\nDATA SIZE MISMATCH - {src_name} {src_type} '
                            f'({src_precision},{src_scale})')
                    elif src_type != 'NUMERIC':
                        print('Data Size Mismatch ', src_name, src_type, src_size, tgt_size, int(src_size) - int(tgt_size))
                        diff_column_str += (f'\nDATA SIZE MISMATCH - {src_name} {src_type}({src_size})')
                else:
                    continue
        ...
        if column_found_ind == False:
            ...
            if src_type not in (
                        'BIGINT', 'FLOAT', 'DATE', 'INTEGER', 'SMALLINT'):
                if src_type == 'NUMERIC':
                    diff_column_str += (f'\nNOT FOUND - {src_name} {src_type}({src_precision},{src_scale})')
                else:
                    diff_column_str += (f'\nNOT FOUND - {src_name} {src_type}({src_size})')             
            else:
                diff_column_str += (f'\nNOT FOUND - {src_name} {src_type}')
