...
class dbConn:
    ...
    def DBConnection(self):
        ...
        try:
            ...
            logging.error("DB Connection Failure {}".format(str(err)))
            raise ()
    ...
    def DBConnection_QB(self):
        ...
        try:            # dsn: Data source name to connect to QuickBooks
            ...
            logging.error("QuickBooks DB Connection Failure {}".format(str(err)))
            raise ()
    ...
    def DBConnection_MySql(self):
        ...
        try:
            ...
            print("Unable to Establish DB Connection -> ()".format(str(err)))
            exit()
    ...
    def selectRowsWithCursorNextFetch(self, fetch_size):
        """
        Execute a fetch next for the passed number of rows.
    ...
    def getColumnNames(self, table_nm):
        ...
        for row in column_rows:
            if self.getDbType() == 'MYSQL':
                ...
                column_list.append([row[3], row[5], row[7], row[6], row[8]])
                #print('*** row -> ', row)
